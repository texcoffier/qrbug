...
async def run(incident: qrbug.Incidents, _request: web.Request) -> Optional[str]:
    ...
    if incident.failure_id == 'SHOW_JOURNALS':
        db_file_contents = await get_html_from_path(qrbug.DB_FILE_PATH)
        default_db_file_contents = await get_html_from_path(qrbug.DEFAULT_DB_PATH)
        incidents_file_contents = await get_html_from_path(qrbug.INCIDENTS_FILE_PATH)
