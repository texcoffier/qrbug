...
async def run(incidents, request):
    ...
    if incident.failure_id == 'dispatcher-action_id':
        ...
        feedback = f"Le nouvel {FULL_BEACON}action_id</{BEACON_NAME}> du dispatcher {FULL_BEACON}{dispatcher_id}</{BEACON_NAME}> est {FULL_BEACON}{html.escape(value)}{FULL_BEACON}.\n"
    elif incident.failure_id == 'dispatcher-selector_id':
        qrbug.append_line_to_journal(
                f'dispatcher_update({repr(dispatcher_id)}, selector_id={repr(value)})\n', qrbug.Journals.DB)
        feedback = f"Le nouveeau {FULL_BEACON}selector_id</{BEACON_NAME}> du dispatcher {FULL_BEACON}{dispatcher_id}</{BEACON_NAME}> est {FULL_BEACON}{html.escape(value)}</{BEACON_NAME}>.\n"
    else:
